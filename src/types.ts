...
function buildCurrentSiteEvidenceInstruction(cmd: BlogCommand, targetTrimmed: string): string {
  if (cmd.key !== 'site-health' && cmd.key !== 'geo') return '';
  const origin = siteOriginFromTarget(targetTrimmed);
  if (!origin) return '';
  return (
    `\n\nUse current-site evidence for all technical availability claims. Before saying any site-level asset, ` +
    `directive, metadata, schema, feed, sitemap, crawler rule, AI-readiness file, or integration is missing, outdated, ` +
    `or absent, directly verify the relevant current URL/source and name what you checked. For this site, explicitly ` +
    `check at minimum \`${origin}/robots.txt\`, \`${origin}/sitemap.xml\`, and \`${origin}/llms.txt\` when discussing ` +
    `crawlability, indexation, or AI citation readiness. Treat any successfully fetched valid file or markup as present; ` +
    `separate "present but incomplete/not optimized" from "missing". If a file cannot be fetched, report it as "could not verify" ` +
    `with the attempted URL/status instead of asserting it does not exist. Include a short "Verification evidence" note in the report ` +
    `for high-impact technical findings.`
  );
}
...
export function buildBlogPrompt(
  ...
  return `${cmd.command} ${t}${buildCurrentSiteEvidenceInstruction(cmd, t)}\n\n${instruction}`.trim();
}
