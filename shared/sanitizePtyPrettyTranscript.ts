...
export function stripPtyEphemeralLines(text: string): string {
  const spinner =
    /(?:Undulating|Thinking|Bouncing|Pulsing|Compacting|Scribbling|Catapulting|Warping|Drizzling|Twirling|Cerebrating|Percolating|Simmering|Ruminating|Marinating|Brooding|Festooning|Moonwalking)/i;
  return text
    .split('\n')
    .map((line) => {
      ...
      if (/✻/.test(line)) {
        let s = line.replace(
          /\s*✻\s*(?:Undulating|Thinking|Bouncing|Pulsing|Compacting|Scribbling|Catapulting|Warping|Drizzling|Twirling|Cerebrating|Percolating|Simmering|Ruminating|Marinating|Brooding|Festooning|Moonwalking)[·….\s]*/gi,
          ''
        );
