...
export function plainTextShowsClaudePermissionMenu(plainNormalized: string): boolean {
  ...
  if (!/\bEsc to cancel\b/i.test(tail)) return false;
  /** Ink sometimes tweaks the Tab hint line (“amend” vs “edit”, middle-dot separators). */
  if (!/\bTab to (?:amend|edit|change)\b/i.test(tail)) return false;
  /** Numbered row: allow tight `1.Yes` or `> 1. Yes` style lines. */
...
function menuOptionSlice(plainNormalized: string): string {
  ...
  const proceed = /\bDo you want to proceed\??/i.exec(menuBody);
  const sliceStart =
